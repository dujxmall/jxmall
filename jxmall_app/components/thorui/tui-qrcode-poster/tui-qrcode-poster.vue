<template>
	...
	<view class="content" v-if="isShow" @click.stop="isShow=false">
		...
		#
		<!-- #ifdef H5 -->
		<view class="save-btn" @click.stop="saveImage">长按图片另存为</view>
		<!-- #endif -->
		<!-- #ifdef MP-WEIXIN -->
		...
		methods: {
			...
			async __init() {
				...
				this.ctx.fillText(this.price, (((this.canvasW - hW) / 2) + 18), (
					((this.canvasW - hW) / 2) + hH + 90))
				//绘制虚线
				this.drawDashLine(this.ctx, 10, (((this.canvasW - hW) / 2) + hH + 120), (this.canvasW - 10), (((this.canvasW - hW) /
					2) + hH + 120), 5)
				//左边实心圆
				...
				this.ctx.drawImage(qrcodeImg.path, 198, (((this.canvasW - hW) / 2) + hH + 135), 82, 82)
				//延迟渲染
				setTimeout(() => {
					this.ctx.draw(true, () => {
						uni.hideLoading()
