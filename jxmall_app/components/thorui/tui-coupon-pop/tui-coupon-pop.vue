		...
		<view class="tui-coupon-list">
			<view class="tui-coupon-item tui-top20" v-for="(item,index) in list" :key="index">
				...
				<view class="tui-btn-box" v-if="item.status==0">
					<tui-button type="danger" width="152rpx" height="52rpx" :size="24" shape="circle" v-if="select_index!=index"
					 @click="use(item,index)">使用</tui-button>
					<tui-button type="danger" width="152rpx" height="52rpx" :size="24" shape="circle" :plain="true" v-if="select_index==index">已选择</tui-button>
				</view>
			</view>
		...
		methods: {
			use(coupon, index) {
				...
				this.$emit('selected', {
					...
					use_coupon_goods_id: coupon.goods_id
				})
