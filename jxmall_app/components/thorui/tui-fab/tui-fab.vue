				...
				<view class="tui-fab-item-box" :class="{'tui-fab-item-left':left && !right && item.imgUrl}"
					...
					<view class="tui-fab-item"
						...
						<view class="tui-fab-title" v-if="!item.imgUrl"
							:style="{fontSize:item.fontSize+'rpx',color:item.color}">{{item.text || ""}}</view>
						<tui-button :openType="item.openType" shape="circle" :width="item.imgWidth+'rpx'"
							:height="item.imgHeight+'rpx'" v-else>
							<image :src="item.imgUrl" class="tui-fab-img" mode="aspectFill"
								:style="{width:item.imgWidth+'rpx',height:item.imgHeight+'rpx',borderRadius:radius}"></image>
						</tui-button>
					</view>
				</view>
			</view>
