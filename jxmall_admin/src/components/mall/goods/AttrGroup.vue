<template>
	<div class="attr-group">
		<div v-for="(group, i) in value" :key="i" style="margin-bottom: 16px">
			<div flex="dir:left cross:center" class="bg flex-row flex-y-center">
				<span>规格名:</span>
				<el-input type="text" v-model.trim="group.attr_group_name" size="mini" @input="makeAttrGroup" style="width:94px;margin:0 16px"></el-input>
				<el-checkbox v-if="i === 0" v-model="attrPic">规格图片</el-checkbox>
				<div @click="deleteAttrGroup(i)" style="margin-left: auto;height: 100%;" class="flex-y-center">
					<el-image class="del-img" :src="delImg"></el-image>
				</div>
			</div>
			<div flex="dir:left" style="padding:0 10px;margin-top: 16px">
				<div class="box-grow-0">规格值：</div>
				<draggable :options="{draggable:'.attr-list'}" @end="makeAttrGroup" flex="dir:left" style="flex-wrap: wrap;display: flex;" v-model="group.attr_list">
					<div class="attr-list" v-for="(attr, j) in group.attr_list" :key="j">
						<el-input style="width:152px" size="mini" type="text" @input="makeAttrGroup" v-model.trim="attr.attr_name"></el-input>
						...
						<div flex="cross:center main:center" class="img-box" v-if="i===0 && attrPic">
							...
							<div v-if="attr.pic_url" @mouseenter="attrPicEnter(j)" class="flex-y-center flex-x-center">
								<el-image :src="attr.pic_url" style="height:80px;width:80px"></el-image>
							</div>
							<file-picker v-else v-model="attr.pic_url" :multiple="false" :max="1" style="width: 100px;height: 100px;"  class="flex-x-center flex-y-center">
								<el-tooltip class="item" effect="dark" content="建议尺寸:750 * 750" placement="top">
									<el-button type="text">+添加图片</el-button>
								</el-tooltip>
							</file-picker>
							<i v-if="attrPicStatus === j && attr.pic_url" class="el-icon-error close" @click="attr.pic_url = ''"></i>
							<div v-if="attrPicStatus === j && attr.pic_url" style="position: absolute">
								<file-picker v-model="attr.pic_url" :multiple="false" :max="1">
...
<script>
	...
	import delImg from '@/assets/statics/mall/del.png';
	export default {
		components: {
			draggable,
			FilePicker,
