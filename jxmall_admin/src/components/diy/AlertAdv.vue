<template id="test">
	...
	<el-dialog :visible.sync="visible" width="60%" height="60%">
		<div class="container flex-row">
			<div class="diy-component-preview">
				<div class="content" style="width: 750px;background-color: #F0F0F0;border: solid #DDDDDD;padding: 10px;background: rgba(0,0,0,0.6);zoom: 0.5;">
					<div style="width: 100%;height: 100%;position: relative;">
						<div class="img-container">
							<div style="position: relative;text-align: center;width: 100%;height: 100%;">
								<el-image :src="data.bg.url" style="margin: 0 auto;overflow: hidden;" :style="{top:data.bg.top+'%',height:(data.bg.height+'%'),borderRadius:data.bg.radius+'px',width:data.bg.width+'%'}"
								 fit="fill"></el-image>
							</div>
						</div>
						<div style="z-index: 9999999999;position: absolute;width: 100%;left: 0;" :style="{top:data.btn.top+'%'}" class="flex-y-center flex-x-center">
							<div style="height: 100px;overflow: hidden;border: solid #f0f0f0 1px;" :style="{borderColor:data.btn.borderColor,width:data.btn.width+'%',borderRadius:data.btn.radius+'px',backgroundColor:data.btn.bgColor,color:data.btn.color,fontSize:data.btn.fontSize+'px',height:data.btn.height+'px'}"
							 class="flex-y-center flex-x-center" v-if="data.btn.style==0" @click="btnClick">
								{{data.btn.text}}
							</div>
							<el-image :src="data.btn.btnImg" :style="{width:data.btn.width+'%',borderRadius:data.btn.radius+'px',height:data.btn.height+'px'}"
							 fit="fill" v-if="data.btn.style==1" @click="btnClick"></el-image>
						</div>
					</div>
				</div>
			</div>
			<scroll-bar :height="550">
				<div class="diy-component-edit" style="padding-left: 20px;padding-right: 50px;">
					<el-form label-width="100px">
						<el-form-item label="背景图片">

							<div flex="main:center cross:center" class="add-image-btn flex-x-center flex-y-center" v-if="data.bg.url">
								<img :src="data.bg.url" alt="" style="width: 100%;height: 100%;">
								<span class="flex-x-center flex-y-center pic-del" @click="data.bg.url=''"> <i class="el-icon-close"></i></span>
							</div>
							<file-picker v-if="!data.bg.url" style="margin-right: 5px;" v-model="data.bg.url" width="88" height="88">
						...
						<div style="width: 100%;height: 2px;border-bottom: dashed #DDDDDD 2px;"></div>
						...
						<el-form-item label="按钮图片" v-if="data.btn.style==1">
							<div flex="main:center cross:center" class="add-image-btn flex-x-center flex-y-center" v-if="data.btn.btnImg">
								<img :src="data.btn.btnImg" alt="" style="width: 100%;height: 100%;">
								<span class="flex-x-center flex-y-center pic-del" @click="data.btn.btnImg=''"> <i class="el-icon-close"></i></span>
							</div>
							<file-picker v-if="!data.btn.btnImg" style="margin-right: 5px;" v-model="data.btn.btnImg" width="88" height="88">
								<div flex="main:center cross:center" class="add-image-btn flex-x-center flex-y-center">
									<i class="el-icon-upload"></i>
								</div>
							</file-picker>
			...
			return {
				...
				data: {
					...
					bg: {
						...
						height: 70,
						width: 80,
						url: 'http://jxmall.sinbel.cn/web//uploads/images/original/20201214/1c2ecf81a65da0355e10b1dd6e505a3b.png',
						radius: 30
					},
					btn: {
						type: 0, //0 close 跳转按钮
						...
						url: '',
						fontSize: 35,
						btnImg: 'http://jxmall.sinbel.cn/web//uploads/images/original/20201129/87482e97b53eef30849b38e920750c98.jpeg',
