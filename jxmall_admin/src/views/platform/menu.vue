...
<div class="app-container">
  <el-card class="box-card" v-loading='is_loading'>
    <div slot="header" class="clearfix">
      <span>参数设置</span>
      <el-button style="float: right;padding: 3px 0;" type="text" @click="save">保存并发布</el-button>
    </div>
    <div class="form-body">
            ...
            <div v-for="(item, i) of menu.button" :key="i" class="menu_bottom">
              ...
              <div class="submenu">
                <div v-for="(subItem, k) in item.sub_button" :key="k" class="subtitle">
                  <div :class="{'active': isSubMenuActive == i + '' + k}" class="menu_subItem" @click="subMenuFun(item, subItem, i, k)">{{ subItem.name }}
                  </div>
                </div>
                                    ...
                                    <el-popover v-model="visible2" placement="top">
                                        <el-table :data="tableData" style="width: 100%">
                                            ...
                                            <el-table-column label="操作">
                                                <template slot-scope="scope">
                                                    <el-button size="mini" @click="handleEdit(scope.$index, scope.row)">选择
                                                    </el-button>
                                                </template>
                                            </el-table-column>
                                        </el-table>
	...
	export default {
		...
		methods: {
			...
			getMenu() {
				...
				this.$request({
					url: '/mall/platform/wechat-menu',
				}).then(e => {
					this.is_loading = false;
					if (e.data.code === 0) {
						this.menu = e.data.menu;
					} else {
