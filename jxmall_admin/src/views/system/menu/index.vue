<template>
	<div class="app-container">
		...
		<el-table v-if="refreshTable" v-loading="loading" :data="menuList" row-key="menu_id"
			:default-expand-all="isExpandAll" :tree-props="{children: 'children', hasChildren: 'hasChildren'}">
			<el-table-column prop="menu_id" label="菜单id" :show-overflow-tooltip="true"></el-table-column>
			...
			<el-table-column prop="order_num" label="显示" width="60">
				<template slot-scope="scope">
					<div>
						<el-tag v-if="scope.row.hidden==1" type='warning'>隐藏</el-tag>
						<el-tag v-if="scope.row.hidden==0" type='success'>显示</el-tag>
					</div>
				</template>
			</el-table-column>
			<el-table-column prop="active_menu" label="显示菜单">
				<template slot-scope="scope">
					<div>
						<div v-if="scope.row.active_menu">
							<el-tag type='success'>{{scope.row.active_menu}}</el-tag>
						</div>
						<div v-else>-</div>
					</div>
				</template>
			...
			<el-table-column prop="name" label="组件名称" :show-overflow-tooltip="true"></el-table-column>
				...
				<el-row>
					<el-col :span="24">
						<el-form-item label="上级菜单">
							<treeselect v-model="form.parent_id" :options="menuOptions" :normalizer="normalizer"
					...
					<el-col :span="12" v-if="form.menu_type != 'F'">
						<el-form-item prop="name">
							<span slot="label">
								<el-tooltip content="组件的name属性非文件名称，命名规则，例如组件路径：jxmall/demo/demo,组件名称为：jxmall-demo-demo"
									placement="top">
					...
					<el-col :span="12" v-if="form.menu_type != 'F'">
						<el-form-item prop="plugin">
							<span slot="label">
								<el-tooltip content="属于哪个插件的菜单，默认mall" placement="top">
									<i class="el-icon-question"></i>
								</el-tooltip>
								插件标识
							</span>
							<el-select style="width: 100%;" v-model="form.plugin" clearable placeholder="请选择">
								<el-option v-for="item in plugin_map" :key="item.name" :label="item.label"
									:value="item.name">
								</el-option>
							</el-select>
						</el-form-item>
					...
					<el-col :span="12">
						<el-form-item label="显示排序" prop="order_num">
							<el-input-number style="width: 100%" v-model="form.order_num" controls-position="right"
								:min="0" />
						</el-form-item>
					</el-col>
					...
					<el-col :span="12" v-if="form.menu_type != 'M'">
						<el-form-item>
							<el-input v-model="form.active_menu" placeholder="高亮菜单" maxlength="100" />
							<span slot="label">
								<el-tooltip content="设置了高亮菜单的路由,当在该页面时候,将点亮高亮菜单" placement="top">
									<i class="el-icon-question"></i>
								</el-tooltip>
								高亮菜单
							</span>
					...
					<el-col :span="12" v-if="form.menu_type != 'F'">
						<el-form-item>
							<span slot="label">
								<el-tooltip content="是否在面包屑中显示" placement="top">
									<i class="el-icon-question"></i>
								</el-tooltip>
								面包屑显示
							</span>
					...
					<el-col :span="12" v-if="form.menu_type != 'F'">
						<el-form-item>
							<span slot="label">
								<el-tooltip content="是否显示在标签栏" placement="top">
									<i class="el-icon-question"></i>
								</el-tooltip>
								显示标签
							</span>
							<el-radio-group v-model="form.show_tag">
...
<script>
	...
	export default {
		...
		methods: {
			...
			getPluginList() {
				this.$request({
					url: "/admin/admin/mall-plugin"
				}).then(e => {
					if (e.code == 0) {
						this.plugin_map = this.plugin_map.concat(e.data.list);
					}
				})
			},
			...
			/** 删除按钮操作 */
			handleDelete(row) {
				this.$modal.confirm('是否确认删除名称为"' + row.menu_name + '"的数据项？').then(function() {
